...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['id'])) {
    try {
        ...
        $insertSql = "INSERT INTO kelas (name, jenjang_id) VALUES (?, ?)";
        $insertStmt = $conn->prepare($insertSql);
        $insertStmt->bind_param("si", $name, $jenjang_id);
        $insertStmt->execute();
        $insertStmt->close();

        $_SESSION['message_success'] = 'Data Kelas Berhasil ditambahkan';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['id']) {
    try {
        ...
        $updateSql = "UPDATE kelas SET name=?, jenjang_id=?  WHERE id=?";
        $updateStmt = $conn->prepare($updateSql);
        $updateStmt->bind_param("sii", $name, $jenjang_id, $id);
        $updateStmt->execute();
        $updateStmt->close();


        $_SESSION['message_success'] = 'Data Kelas Berhasil diubah';
        header("Location: data_kelas.php");
        exit();
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Data Master Kelas</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item"><a href="data_kelas.php">Master Kelas</a></li>
                            <li class="breadcrumb-item active">List Master Kelas</li>
                        </ol>
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <div class="card mb-4">
                                    <div class="card-header">
                                        ...
                                        <div class="col-lg-12 d-flex justify-content-between align-items-center">
                                                ...
                                                <i class="fas fa-plus"></i> Tambah Data Master Kelas
                                                </button>
                                                <a href="<?= 'export_kelas.php' ?>" id="exportExcelBtn" class="btn btn-success">
                                    ...
                                    <table id="datatablesSimple">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $data['name']; ?></td>
                                                <td>
                                                    <a href="javascript:void(0)" data-url="<?= 'show_kelas.php' ?>" data-id="<?= $data['id'] ?>" class="edit btn btn-info btn-sm text-light "> <i class="fas fa-edit"></i></a>
                                                    <a href="javascript:void(0)" data-url="<?= 'delete_kelas.php' ?>" data-id="<?= $data['id'] ?>" class="delete btn btn-danger btn-sm"> <i class="fas fa-times"></i></a>
                                                </td>
                                            </tr>
                                            <?php
                                                }
                ...
                <?php include 'create_kelas.php' ?>
                <?php include 'edit_kelas.php' ?>
            ...
            $(function () {
                ...
                $('#datatablesSimple tbody').on('click', '.edit', function () {
                    ...
                    $.ajax({
                       ...
                       if(response.status){
                            let data = response.data.kelas;
                            let dataJenjang = response.data.jenjang;
                            $('#id_kelas_edit').val(data.id);
                            $('#name_edit').val(data.name);
                            let html = '<option value="">Pilih Jenjang</option>';
                            dataJenjang.forEach((jenjang, index) => {
                                html += `<option value="${jenjang.id}" ${jenjang.id == data.jenjang_id ? 'selected' : ''}>${jenjang.name}</option>`
                            });
